...
export const checkAndSendAlerts = actionGeneric({
  ...
  handler: async (ctx, { siteId }) => {
    ...
    if (
      ...
      await sendEmail(resendKey, user.email,
    ...
    if (site.alertConfig.criticalFindings) {
      const prevCriticalIds = new Set(((prev.findings ?? []) as any[]).filter((f: any) => f.severity === 'critical').map((f: any) => f.id ?? f.title))
      const newCritical = ((curr.findings ?? []) as any[]).filter((f: any) => f.severity === 'critical' && !prevCriticalIds.has(f.id ?? f.title))
    ...
    if (site.alertConfig.crawlerBlocked) {
      const prevBlocked = new Set(((prev.robotsPatch ?? []) as any[]).filter((r: any) => r.blocked).map((r: any) => r.bot))
      const newlyBlocked = ((curr.robotsPatch ?? []) as any[]).filter((r: any) => r.blocked && !prevBlocked.has(r.bot))
